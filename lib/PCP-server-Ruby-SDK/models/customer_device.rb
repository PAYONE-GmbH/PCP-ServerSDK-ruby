  ...
  class CustomerDevice
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PCPServerSDK::CustomerDevice` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PCPServerSDK::CustomerDevice`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@device_token.nil? && @device_token.to_s.length > 255
        invalid_properties.push('invalid value for "device_token", the character length must be smaller than or equal to 255.')
      end
    ...
    def valid?
      ...
      return false if !@device_token.nil? && @device_token.to_s.length > 255
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] device_token Value to be assigned
    def device_token=(device_token)
      if device_token.nil?
        fail ArgumentError, 'device_token cannot be nil'
      end

      if device_token.to_s.length > 255
        fail ArgumentError, 'invalid value for "device_token", the character length must be smaller than or equal to 255.'
      end
